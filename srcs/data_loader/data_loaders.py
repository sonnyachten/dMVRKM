...
from srcs.utils.util import standardize
...
def belgian_energy_consumption_2020_Dataloader(data_dir, max_nr=5000):
    name = f"{data_dir}/ods001.csv"
    if not os.path.exists(name):
        print("Data at the given path doesn't exist. Downloading now...")
        wget.download(
            "https://external-elia.opendatasoft.com/explore/dataset/ods001/download/?format=csv&refine.datetime=2020&timezone=Europe/Brussels&lang=en&use_labels_for_header=true&csv_separator=%3B",
            data_dir,
        )
    data = pd.read_csv(name, header=0, sep=";")
    dataset = torch.tensor(data["Total Load"].values[:max_nr])
...
def lorentz_Dataloader(max_nr=2000, plot3d=False, s=10, r=28, b=2.667):
    dt = 0.01
    num_steps = max_nr
...
def pre_process_data(data, lag):
    """
    Pre-process the data to convert it into time-lagged format and standardize it.
    :param lag:
    :param data:
    :return: Standardized data and labels.
    """
    std = standardize()
    window_length = data.shape[0] - lag - 1  # -1 because of the Y is also computed here
    X = torch.zeros((window_length, lag + 1 + 1))
    for i in range(X.shape[1]):
        X[:, i] = data[i : i + window_length]

    X = std(X)[0]
    return X[:, :-1], X[:, -1].reshape((-1, 1))  # returns X and Y
