...
class Multiview_RKM_NAR_Dual:
    ...
    def train(self, X: torch.Tensor, Y: torch.Tensor):
        ...
        if self.kwargs["decomposition_method"] == "svd":
            ...
            lambdas, self.H = S[: self.s], H[:, : self.s]
        ...
        torch.save(self.H, "H_dual.pt")
        torch.save(lambdas, "dual_lam.pt")
        ...
        self.coeff_h_y = torch.linalg.inv(self.lambdas - (self.H.T @ KY @ self.H))
    ...
    def predict_y(self, init_vec: torch.tensor = None, n_steps: int = 1, verbose=True):
        ...
        for i in conditional_trange(verbose, n_steps, desc="Predicting"):
            ...
            if self.kwargs_y["kernel"] == "linear":
                ...
                op = self.rr(self.X, self.H_pred, self.Y_pred)
            else:
                raise ValueError("pre_image_method not valid.")

            self.Y_pred = torch.cat((self.Y_pred, op.unsqueeze(0)), dim=0)
    ...
    def predict_x(self, init_vec: torch.tensor = None, n_steps: int = 1, verbose=False):
        ...
        for i in conditional_trange(verbose, n_steps, desc="Predicting"):
            ...
            if self.kwargs_x["kernel"] == "linear":
                ...
                op = self.rr(self.X, self.H, self.Y)
            else:
                raise ValueError("pre_image_method not valid.")
    ...
    def compute_kernel(self, X: torch.Tensor = None, x: torch.Tensor = None, **kwargs):
        ...
        if kwargs["kernel"] == "rbf":
            if "approximator" in kwargs:
                if kwargs["approximator"] == "rff":
                    gamma = 0.5 * 1 / kwargs["sigma"] ** 2
                    self.kernel_x_sampler = RBFSampler(
                        n_components=kwargs["DIM_FEATURES"], gamma=gamma, random_state=42
                    )  # gamma=5e-4
                    features_X = torch.tensor(self.kernel_x_sampler.fit_transform(X))
                    features_x = torch.tensor(self.kernel_x_sampler.fit_transform(x))
                    return (features_X @ features_x.T).squeeze()
            else:
                sqd = (
                        torch.cdist(X, x, p=2) ** 2
